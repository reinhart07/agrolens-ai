...
Load semua model ML saat startup FastAPI
"""
...
def load_all_models():
    ...
    try:
        ...
        if os.path.exists(xgb_path):
            ...
            print("⚠️  model_price_xgb.pkl tidak ditemukan")
    ...
    try:
        ...
        if os.path.exists(fc_path):
            ml_models["feature_cols_harga"] = joblib.load(fc_path)
            print(f"✅ Feature cols loaded: {ml_models['feature_cols_harga']}")

        print("✅ Encoder & Scaler Harga loaded!")
    except Exception as e:
        print(f"⚠️  Encoder/Scaler Harga: {e}")

    # ── Model Kualitas (MobileNetV2) ──────────────────────────
    try:
        import tensorflow as tf
