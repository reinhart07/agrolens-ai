...
import {
  LayoutDashboard, CreditCard, Users, TrendingUp,
  FileText, Bell, LogOut, Menu, Leaf,
  ChevronRight, User, Settings
} from 'lucide-react'

const navItems = [
  { to: '/mitra/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/mitra/kredit',    icon: CreditCard,      label: 'Credit Scoring' },
  { to: '/mitra/petani',    icon: Users,           label: 'Data Petani' },
  { to: '/mitra/harga',     icon: TrendingUp,      label: 'Monitor Harga' },
  { to: '/mitra/laporan',   icon: FileText,        label: 'Laporan' },
  { to: '/mitra/settings',  icon: Settings,        label: 'Pengaturan' },
]
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-agro-dark border-r border-white/10 transform transition-transform duration-300
        ...
        <div className="flex items-center gap-3 px-6 py-5 border-b border-white/10">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-amber-400">Dashboard Mitra</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-agro-dark/90 backdrop-blur border-b border-white/10 px-4 lg:px-8 py-4 flex items-center gap-4">
          ...
          <button className="relative text-gray-400 hover:text-white">
            ...
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-amber-500 rounded-full border-2 border-agro-dark" />
          </button>
