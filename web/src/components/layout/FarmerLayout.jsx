...
import { LayoutDashboard, ShoppingBag, TrendingUp, CreditCard, MapPin, MessageCircle, FileText, Bell, LogOut, Menu, Leaf, ChevronRight, User, Camera, Package, Crown } from 'lucide-react'

const navItems = [
  { to:'/farmer/dashboard', icon:LayoutDashboard, label:'Dashboard' },
  { to:'/farmer/komoditas', icon:ShoppingBag,     label:'Komoditas Saya' },
  { to:'/farmer/pesanan',   icon:Package,         label:'Pesanan' },
  { to:'/farmer/harga',     icon:TrendingUp,      label:'Prediksi Harga' },
  { to:'/farmer/kredit',    icon:CreditCard,      label:'Credit Score' },
  { to:'/farmer/kualitas',  icon:Camera,          label:'Deteksi Kualitas' },
  ...
  { to:'/farmer/profil',    icon:User,            label:'Profil & Rekening' },
  { to:'/farmer/premium',   icon:Crown,           label:'Premium ⭐', premium:true },
]
    ...
    <div className="min-h-screen bg-agro-dark flex">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-agro-dark border-r border-white/10 transform transition-transform duration-300 ${sidebarOpen?'translate-x-0':'-translate-x-full'} lg:translate-x-0 lg:static lg:block`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.to} to={item.to} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${item.premium ? active ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-amber-400/70 hover:bg-amber-500/10 hover:text-amber-400' : active ? 'bg-agro-green/20 text-agro-green border border-agro-green/30' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/10">
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-gray-400 hover:bg-red-500/10 hover:text-red-400 transition-all">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-agro-dark/90 backdrop-blur border-b border-white/10 px-4 lg:px-8 py-4 flex items-center gap-4">
          ...
          <button className="relative text-gray-400 hover:text-white"><Bell className="w-5 h-5"/><span className="absolute -top-1 -right-1 w-3 h-3 bg-agro-green rounded-full border-2 border-agro-dark"/></button>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-agro-green/20 rounded-full flex items-center justify-center"><User className="w-4 h-4 text-agro-green"/></div>
